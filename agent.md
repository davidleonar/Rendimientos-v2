...
- **Admin Dashboard Portal (`/admin` page)**: A secure management console built exclusively for the admin user (`5XgksHrgmyeGqqKFYGVjQVM0KGl1`). Key features include:
  ...
  - **Profiles Search Engine**: Allows instant query searches by UID, Name, or National ID (Cédula), displaying real-time profile summaries, live yield calculations, total COP invested, average purchase price, and the complete chronological list of deposit and withdrawal items.
