const http = require('http');
const https = require('https');
const fs = require('fs');
const url = require('url');

// Define known macaroons (from your index.js and curl tests)
const MAIN_MACAROON = '';
const EDGE_MACAROON = '';

// Node configurations (IPs from your curl tests)
const NODE_CONFIGS = {
  main: {
    hostname: '100.103.9.71',
    port: 8080,
    caPath: './mainnode/tls.cert',
  },
  edge: {
    hostname: '100.68.2.83',
    port: 8080,
    caPath: './edgenode/tls.cert',
  },
};

const server = http.createServer((req, res) => {
  const parsedUrl = url.parse(req.url, true);

  // Extract macaroon from header (Node.js lowercases headers)
  const macaroon = req.headers['grpc-metadata-macaroon'];
  if (!macaroon) {
    res.writeHead(400, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: 'Missing Grpc-Metadata-macaroon header' }));
    return;
  }

  // Determine target based on macaroon match
  let targetConfig;
  if (macaroon === MAIN_MACAROON) {
    targetConfig = NODE_CONFIGS.main;
  } else if (macaroon === EDGE_MACAROON) {
    targetConfig = NODE_CONFIGS.edge;
  } else {
    res.writeHead(400, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: 'Invalid or unknown macaroon' }));
    return;
  }

  // Load the node-specific CA cert
  let agent;
  try {
    const caCert = fs.readFileSync(targetConfig.caPath);
    agent = new https.Agent({ ca: caCert });
  } catch (err) {
    console.error('Error loading CA cert:', err);
    res.writeHead(500, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: 'Failed to load TLS certificate' }));
    return;
  }

  // Build proxy options
  const options = {
    hostname: targetConfig.hostname,
    port: targetConfig.port,
    path: parsedUrl.path,
    method: req.method,
    headers: req.headers,  // Forward all headers, including macaroon
    agent,
  };

  // Forward the request
  const proxyReq = https.request(options, (proxyRes) => {
    res.writeHead(proxyRes.statusCode, proxyRes.headers);
    proxyRes.pipe(res, { end: true });
  });

  req.pipe(proxyReq, { end: true });

  proxyReq.on('error', (err) => {
    console.error('Proxy request error:', err);
    res.writeHead(500, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: err.message }));
  });
});

server.listen(3000, () => console.log('Proxy on 3000'));